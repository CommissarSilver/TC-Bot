...
class nlg:
    ...
    def post_process(self, pred_template, slot_val_dict, slot_dict):
        ...
        for slot in slot_val_dict.keys():
            ...
            if slot == "result" or slot == "numberofpeople":
                continue
            if slot_vals == dialog_config.NO_VALUE_MATCH:
                continue
    ...
    def convert_diaact_to_nl(self, dia_act, turn_msg):
        ...
        if dia_act["diaact"] in self.diaact_nl_pairs["dia_acts"].keys():
            for ele in self.diaact_nl_pairs["dia_acts"][dia_act["diaact"]]:
                if set(ele["inform_slots"]) == set(
                    dia_act["inform_slots"].keys()
                ) and set(ele["request_slots"]) == set(dia_act["request_slots"].keys()):
                    sentence = self.diaact_to_nl_slot_filling(
                        dia_act, ele["nl"][turn_msg]
                    )
                    boolean_in = True
                    break
    ...
    def translate_diaact(self, dia_act):
        ...
        pred_ys, pred_words = self.model.beam_forward(
            inverse_word_dict, dia_act_rep, self.params, predict_model=True
        )
    ...
    def load_nlg_model(self, model_path):
        ...
        if model_params["params"]["model"] == "lstm_tanh":  # lstm_tanh
            ...
            rnnmodel = lstm_decoder_tanh(
                diaact_input_size, input_size, hidden_size, output_size
            )
        ...
        self.inverse_word_dict = {
            self.template_word_dict[k]: k for k in self.template_word_dict.keys()
        }
