...
class AgentCmd(Agent):
    ...
    def parse_str_to_diaact(self, string):
        ...
        if act in self.act_set:  # dialog_config.all_acts
            ...
            print(
                "Something wrong for your input dialog act! Please check your input ..."
            )
                    ...
                    if annot_slot == "result":
                        ...
                        if len(annot_val.strip(" ")) > 0:
                            for result_annot_seg_item in result_annot_seg_arr:
                                result_annot_seg_arr = result_annot_seg_item.strip(
                                    " "
                                ).split("=")
                                ...
                                if result_annot_seg_slot_val == "UNK":
                                    act_slot_value_response["request_slots"][
                                        result_annot_seg_slot
                                    ] = "UNK"
                                else:
                                    act_slot_value_response["inform_slots"][
                                        result_annot_seg_slot
                                    ] = result_annot_seg_slot_val
                        else:  # result={}
                            pass
                        ...
                        for annot_val_ele in annot_val_arr:
                            act_slot_value_response["inform_slots"][annot_slot].append(
                                annot_val_ele
                            )
                else:  # single choice
